...
function normaliseGeo(value){return String(value||'').toLowerCase().normalize('NFKD').replace(/[^a-z0-9 ]/g,' ').replace(/\b(county|co|ireland|republic|city)\b/g,' ').replace(/\s+/g,' ').trim();}
...
function wastewaterRelevance(item){
  if(state.countryCode!=='IE'||item.source_kind!=='wastewater_surveillance')return 0;
  const county=normaliseGeo(item.county),subdivision=normaliseGeo(state.subdivision),place=normaliseGeo(state.place),catchment=normaliseGeo(item.catchment);
  if(catchment&&place&&place.includes(catchment))return 3;
  const catchmentCore=catchment.replace(/\b(north|lower|upper|regional|sewerage|scheme)\b/g,' ').replace(/\s+/g,' ').trim();
  if(catchmentCore.length>=4&&place.includes(catchmentCore))return 3;
  if(county&&subdivision&&(subdivision.includes(county)||county.includes(subdivision)))return 2;
  if(county&&place.includes(county))return 1;
  return 0;
}
function wastewaterResultWeight(item){const value=String(item.result_category||'').toLowerCase();if(value==='positive')return 3;if(value.includes('weak'))return 2;if(value.includes('undetect'))return 1;return 0;}
function wastewaterRelevanceLabel(item){const score=wastewaterRelevance(item);if(score>=3)return 'Named catchment match';if(score===2)return 'Same county';if(score===1)return 'County context';return 'Ireland surveillance';}

function publicHealthMatches(){
  const code=state.countryCode;const items=state.publicHealthData?.items||[];if(!code)return {local:[],exact:[],national:[],regional:[],all:[]};
  const recent=items.filter(i=>ageDays(i.published_at)<=90);
  const exact=recent.filter(i=>(i.countries||[]).includes(code));
  const local=exact.filter(i=>i.source_kind==='wastewater_surveillance'&&wastewaterRelevance(i)>0).sort((a,b)=>wastewaterRelevance(b)-wastewaterRelevance(a)||wastewaterResultWeight(b)-wastewaterResultWeight(a));
  const national=exact.filter(i=>i.source_kind==='national_surveillance');
  const exactThreats=exact.filter(i=>!['national_surveillance','wastewater_surveillance'].includes(i.source_kind));
...
function publicHealthSignals(){
  ...
  const wastewaterSignals=m.local.filter(item=>['positive','weak positive'].includes(String(item.result_category||'').toLowerCase())).slice(0,2).map(item=>({key:`wastewater-${item.id}`,level:String(item.result_category||'').toLowerCase()==='positive'?'moderate':'low',title:`Wastewater surveillance · ${item.catchment}`,summary:`HPSC ${item.result_category} SARS-CoV-2 wastewater result matched your ${wastewaterRelevanceLabel(item).toLowerCase()} context. This is population surveillance, not evidence of a nearby patient or personal exposure.`,action:'Use this as local circulation context only. Open the HPSC report for interpretation and follow current health guidance if it is relevant to you.',query:'health centre pharmacy',source:item.source,url:item.url,kind:'local-public-health',publishedAt:item.published_at}));
  return [...outbreakSignals,...wastewaterSignals];
}
...
function renderContext(){
  ...
  const localPositive=m.local.filter(i=>String(i.result_category||'').toLowerCase()==='positive').length;
  const diseaseText=localPositive?`${localPositive} local wastewater detection${localPositive===1?'':'s'}`:m.local.length?'Local wastewater surveillance updated':m.exact.length?`${m.exact.length} matched official notice${m.exact.length===1?'':'s'}`:m.national.length?'National surveillance updated':m.regional.length?'Regional monitoring available':'No recent matched item';
  ...
  els.contextProvenance.innerHTML=`<div><span>Location</span><strong>${safeText(state.countryName||'Country unresolved')} · ${safeText(state.subdivision||'')}</strong></div><div><span>Weather</span><strong>${w?'Live modelled data · Open-Meteo':'Unavailable'}</strong></div><div><span>Public health</span><strong>Official cache · WHO${EEA.has(state.countryCode)?' + ECDC':''}${state.countryCode==='IE'?' + HPSC + wastewater catchments':''} · cache ${safeText(cacheTime)}</strong></div><div><span>National authority</span><strong><a href="${provider.authority_url}" target="_blank" rel="noopener">${safeText(provider.authority_name)}</a></strong></div>`;
  return c;
}
...
function renderVicinity(c){
  ...
  if(m.local.length){const positive=m.local.filter(i=>String(i.result_category||'').toLowerCase()==='positive').length;diseaseStatus=positive?'Wastewater RNA detected':'Local wastewater update';diseaseDetail=`${m.local.length} HPSC catchment result${m.local.length===1?'':'s'} matched your county/catchment context. Your ${radius} km radius is not the wastewater catchment boundary.`;diseaseTone=positive?'moderate':'good';}
...
function renderPublicHealth(){
  const m=publicHealthMatches();const items=m.all.slice(0,10);const provider=countryProvider();
