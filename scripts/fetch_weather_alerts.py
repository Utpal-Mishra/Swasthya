...
def severity(text: str) -> str:
    value = text.lower()
    # MeteoAlarm awareness colours/levels: red/4, orange/3, yellow/2.
    if re.search(r"\b(red|extreme|level\s*4)\b", value):
        return "extreme"
    if re.search(r"\b(orange|severe|level\s*3)\b", value):
        return "severe"
    if re.search(r"\b(yellow|moderate|level\s*2)\b", value):
        return "moderate"
    return "unknown"
...
def hazard(text: str) -> str:
    value = text.lower()
    candidates = [
        ("thunder", "Thunderstorm"),("wind", "Wind"),("snow", "Snow / ice"),
        ("ice", "Snow / ice"),("rain", "Rain"),("flood", "Flood"),
        ("high temperature", "High temperature"),("heat", "High temperature"),
        ("low temperature", "Low temperature"),("cold", "Low temperature"),
        ("fog", "Fog"),("forest fire", "Forest fire"),("avalanche", "Avalanche")
    ]
...
def parse_feed(country_code: str, slug: str) -> tuple[list[dict], dict]:
    ...
    for entry in feed.entries:
        title = strip(entry.get("title"))
        summary = strip(entry.get("summary") or entry.get("description"))
        ...
        items.append({
            ...
            "title": title or hazard(combined),
            "hazard": hazard(combined),
            "severity": level,
            "updated_at": strip(entry.get("updated") or entry.get("published")) or fetched_at,
            "url": link,
...
def existing() -> dict:
    try:
        ...
        return {"generated_at": None, "items": [], "country_status": {}}
...
def main() -> None:
    ...
    for code, slug in COUNTRIES.items():
        parsed, meta = parse_feed(code, slug)
        status[code] = meta
        if meta.get("ok"):
            items.extend(parsed)
        else:
    ...
    payload = {
        ...
        "source": "MeteoAlarm / EUMETNET Atom feeds",
        "licence": "CC BY 4.0",
