(() => {
  ...
  function validate(payload){
    ...
    if(!['1.0','swasthya-wearable-v1'].includes(payload.schema_version)) throw new Error('Unsupported wearable snapshot version.');
  ...
  function normalise(payload){
    ...
    if(payload.metrics){
      ...
      return {
        schema_version:'1.0',
        date:(payload.generated_at||new Date().toISOString()).slice(0,10),
        generated_at:payload.generated_at||new Date().toISOString(),
        source:payload.source||'health_connect',
        data_origin:payload.data_origin||'Wearable summary',
        consent_scope:payload.consent_scope||Object.keys(m).filter(k=>m[k]!=null),
        ...
        mood_anchor:payload.mood_anchor||null
      };
    }
    return {...payload,generated_at:payload.generated_at||`${payload.date||new Date().toISOString().slice(0,10)}T12:00:00Z`,data_origin:payload.data_origin||payload.source||'Wearable summary'};
  }
  ...
  function physiologyContext(s){
    ...
    // Product heuristics relative to a person's own baseline; these are not clinical cut-offs.
    ...
    if(finite(s.recovery?.energy_score) && s.recovery.energy_score<50){recovery++;contributors.push('Samsung Energy Score was below 50.');}
    ...
    else if(recovery+arousal===1){title='Mixed physiological context';text='One available wearable signal differs from your recent baseline. Other explanations such as exercise, illness, temperature, caffeine or ordinary variation may contribute.';}
  ...
  function saveAnchors(items){sessionStorage.setItem(MOOD_KEY,JSON.stringify(items.slice(-40)));}
  function associationText(){
    const items=moodAnchors();
    if(items.length<5) return `Add ${5-items.length} more self-reported mood anchor${5-items.length===1?'':'s'} before Swasthya describes any personal association.`;
    const counts=items.reduce((acc,x)=>{acc[x.mood]=(acc[x.mood]||0)+1;return acc;},{});
    const top=Object.entries(counts).sort((a,b)=>b[1]-a[1])[0];
    return `${items.length} session anchors are available. Your most frequently logged state is ${top[0]} (${top[1]}×). Future longitudinal analysis should compare wearable context around these self-reports rather than infer emotion from physiology alone.`;
  }

  function render(){
    if(!snapshot){
      if(els.status){els.status.textContent='NOT CONNECTED';els.status.className='status-pill ageing';}
      if(els.source)els.source.textContent='No wearable summary loaded';
      if(els.updated)els.updated.textContent='—';
      ['sleep','heart','activity','optional'].forEach(k=>{if(els[k])els[k].textContent='—';});
      if(els.contextTitle)els.contextTitle.textContent='Waiting for wearable context';
      if(els.contextText)els.contextText.textContent='Connect through the Android companion or import a local summary file.';
      if(els.contributors)els.contributors.innerHTML='';
      if(els.association)els.association.textContent=associationText();
      return;
    }
    const c=physiologyContext(snapshot);
    if(els.status){els.status.textContent='CONNECTED';els.status.className='status-pill fresh';}
    if(els.source)els.source.textContent=snapshot.data_origin||snapshot.source;
    if(els.updated)els.updated.textContent=fmtDate(snapshot.generated_at);
    ...
    if(els.heart){
      const r=snapshot.heart?.resting_bpm, avg=snapshot.heart?.average_bpm;
      els.heart.textContent=finite(r)?`${Math.round(r)} bpm resting`:finite(avg)?`${Math.round(avg)} bpm avg`:'—';
    }
    ...
    if(els.optional){
      const bits=[];
      if(finite(snapshot.heart?.hrv_ms))bits.push(`HRV ${Math.round(snapshot.heart.hrv_ms)} ms`);
      if(finite(snapshot.recovery?.sleep_spo2_percent))bits.push(`SpO₂ ${snapshot.recovery.sleep_spo2_percent.toFixed(1)}%`);
      if(finite(snapshot.recovery?.skin_temperature_baseline_delta_c))bits.push(`skin temp ${signed(snapshot.recovery.skin_temperature_baseline_delta_c,'°C')}`);
      if(finite(snapshot.recovery?.energy_score))bits.push(`Energy ${Math.round(snapshot.recovery.energy_score)}`);
      els.optional.textContent=bits.join(' · ')||'—';
    }
    if(els.contextTitle)els.contextTitle.textContent=c.title;
    if(els.contextText)els.contextText.textContent=c.text;
    if(els.contributors)els.contributors.innerHTML=c.contributors.length?c.contributors.map(x=>`<li>${safeText(x)}</li>`).join(''):'<li>No baseline-relative contributor is available yet.</li>';
    if(els.association)els.association.textContent=associationText();
  }
  ...
  function receiveSnapshot(payload){
    try{
      ...
      snapshot=normalise(parsed);
      sessionStorage.setItem(STORAGE_KEY,JSON.stringify(snapshot));
      render();
      return {ok:true};
    }catch(error){
      if(els.contextTitle)els.contextTitle.textContent='Wearable snapshot could not be loaded';
      if(els.contextText)els.contextText.textContent=error.message;
      return {ok:false,error:error.message};
    }
  }

  function clearSnapshot(){snapshot=null;sessionStorage.removeItem(STORAGE_KEY);render();}
  function recordMoodAnchor(mood){
    if(!mood){render();return;}
    if(!ALLOWED_MOODS.has(mood))return;
    const c=snapshot?physiologyContext(snapshot):null;
    const items=moodAnchors();
    items.push({mood,at:new Date().toISOString(),context:c?.title||'No wearable snapshot',sleep_delta:snapshot?.sleep?.baseline_delta_minutes??null,resting_hr_delta:snapshot?.heart?.resting_bpm_baseline_delta??null,hrv_delta:snapshot?.heart?.hrv_baseline_delta??null});
    saveAnchors(items);render();
  }
  ...
  window.SwasthyaWearableBridge={receiveSnapshot,clearSnapshot,recordMoodAnchor,getSnapshot:()=>snapshot};
  render();
})();
